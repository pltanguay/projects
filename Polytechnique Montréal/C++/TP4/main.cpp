...
1.	Nous remarquons que le classe Solde à une méthode sans implémentation, getPrixBase(). Dans le contexte du polymorphisme, 
	...
	Étant une classe abstraite, la classe Solde ne sert qu’à établir la méthode getPrixBase() et l'attribut pourcentageSolde_
	qui seront partagés par toutes les classes dérivées : FlightPassSolde et BilletRegulierSolde. D'ordre plus général, cette classe
	abstraite est importante en polymorphisme pour nous indiquer que des attributs et méthodes seront réutilisés par les classes dérivées.

2.	Pour quelle raison devons-nous déclarer certains destructeurs virtuels? 

	Nous voulons nous assurer que les objets pointeurs appartenant aux classes dérivées soient désalloués complètement,
	c'est-à-dire d'éviter un objet dérivé soit désalloué par la classes mère. Ceci aurait pour effet de ne pas désallouer 
	la mémoire complètement et causer une fuit. En déclarant certains destructuers virtuels, nous nous assurons que les pointeurs 
	seront désallouer par les classes dérivées. Les classes de bases polymorphes devraient toujours comporter un destructeur virtuel.
