/*
Question implémentation .h : Quand le compilateur voit l’utilisation d’une classe générique, il doit avoir accès à la définition et l’implémentation pour produire une version spécifique de la classe!
...
int main() {
	...
	*alex -= g_coupons->getConteneur()[1];
	...
	tests.push_back(g_membres->trouverBilletParIntervalle(alex, 800.0, 10000.0).size() == 3 && g_membres->trouverBilletParIntervalle(alex, 600.0, 830.0).size() == 0);
